...
(function () {
  ...
  function playBus() {
    ...
    map.fitBounds(poly.getBounds(), { padding: [60, 60], maxZoom: 16 });
      ...
      $("#busStatus").textContent = "播放失败：" + e.message;
    }
  }
  ...
  let reportOn = false, reportLayers = [], reportPanel = null, reportAnimId = null, reportMarker = null;
  function stopReportAnim() {
    if (reportAnimId !== null) { cancelAnimationFrame(reportAnimId); reportAnimId = null; }
  ...
  function openReport() {
    ...
    reportBtn.textContent = "■ 停止";
    reportStatus.textContent = "准备中…";
    try {
      const steps = REPORT_STEPS.filter((s) => poiById[s.poi]);
      const pts = steps.map((s) => [poiById[s.poi].lat, poiById[s.poi].lon]);
      const gcj = pts.map((p) => wgs2gcj(p[0], p[1]));
      const line = L.polyline(gcj, { color: "#c9a227", weight: 4, dashArray: "6 8", opacity: 0.85 });
      reportLayers.push(line);
      line.addTo(map);
      steps.forEach((s) => markers[s.poi].setIcon(pinIcon(poiById[s.poi], true)));
      ...
      reportMarker = L.marker(gcj[0], {
        icon: L.divIcon({ className: "", html: '<span class="bus-dot" style="--bus:#c9a227"></span>', iconSize: [20, 20], iconAnchor: [10, 10] }),
        interactive: false,
      });
      map.addLayer(reportMarker);
      let total = 0;
      const cum = [];
      for (let i = 1; i < gcj.length; i++) { total += haversine(pts[i - 1], pts[i]); cum.push(total); }
      const dur = Math.max(5, Math.min(14, total / 70));
      const t0 = performance.now();
      function frame(now) {
        const p = Math.min(1, (now - t0) / (dur * 1000));
        const target = p * total;
        let k = 0;
        while (k < cum.length - 1 && cum[k] < target) k++;
        const prev = k === 0 ? 0 : cum[k - 1];
        const seg = cum[k] - prev || 1;
        const f = Math.min(1, (target - prev) / seg);
        const lat = pts[k][0] + (pts[k + 1][0] - pts[k][0]) * f;
        const lon = pts[k][1] + (pts[k + 1][1] - pts[k][1]) * f;
        reportMarker.setLatLng(wgs2gcj(lat, lon));
        reportStatus.textContent = "▶ " + steps[Math.min(k, steps.length - 1)].title;
        if (p >= 1) {
          reportStatus.textContent = "✅ 报到动线播放完成 · " + steps[steps.length - 1].title;
          if (reportMarker) { map.removeLayer(reportMarker); reportMarker = null; }
          reportAnimId = null;
          return;
        }
        reportAnimId = requestAnimationFrame(frame);
      }
      reportAnimId = requestAnimationFrame(frame);
    } catch (e) {
      reportStatus.textContent = "播放失败：" + e.message;
    }
  }
